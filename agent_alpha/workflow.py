...
class AgentAlphaWorkflow:
    ...
    def __init__(
        ...
        self.blueprint_agent = create_agent(
            ...
            system_prompt=(
                ...
                "Use only feature names returned by list_feature_catalog. "
                "Do not invent feature names. "
                "Use only operators from list_combine_operators. "
                "In combine trees, allowed node types are only: component, const, call. "
                "Forbidden: var nodes and raw columns (is_spx, $open, $high, $low, $close, $volume). "
                "When using WHERE(condition, a, b), ensure a and b are numeric score values (not booleans). "
                "Avoid degenerate thresholds such as GT(RANK(x), 0). "
                "Before final output, self-check these constraints. "
    ...
    def _node_blueprint(self, state: AgentAlphaState) -> dict[str, Any]:
        ...
        prompt = (
            ...
            "0) Call list_feature_catalog and list_combine_operators first.\n"
            ...
            "   - Allowed node types are only: component, const, call.\n"
            "   - Forbidden: {'type':'var', ...} and raw columns (is_spx, $open, $high, $low, $close, $volume).\n"
            "   - For WHERE, the first arg may be a numeric score; compiler interprets it as (score > 0).\n"
            "   - WHERE(value_if_true/value_if_false) must be numeric scores, not booleans.\n"
            "   - Avoid trivial guards like GT(RANK(x), 0); use meaningful thresholds (e.g., 0.2/0.5/0.8).\n"
            "3) Keep trees shallow and avoid unnecessary nesting.\n"
            "4) Use only deterministic numeric logic.\n"
            "5) Before final output, self-check:\n"
            "   - No var nodes.\n"
            "   - No raw columns in combine.\n"
            "   - All component features are catalog names.\n"
            "   - All operators are from list_combine_operators.\n\n"
